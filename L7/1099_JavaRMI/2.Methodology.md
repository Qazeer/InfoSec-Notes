...
###### Using Java's RMI registry List()

The remote objects bound in a `Java RMI registry` may be enumerated using the
`list()` method of the `java/rmi/registry/RegistryImpl_Stub` (as implemented by
`Metasploit` and `nmap`) or `java.rmi.registry.LocateRegistry` classes.
...
The `nmap` `NSE` script `rmi-dumpregistry` and the `Metasploit` module
`auxiliary/gather/java_rmi_registry` can be used to attempt to list the objects
bound in a `Java RMI registry`:
