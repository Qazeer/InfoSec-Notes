...
Invoke-Bloodhound -Verbose -Domain '<DOMAIN>' -DomainController '<DC_IP | DC_HOSTNAME>' -LDAPUsername '<USERNAME>' -LDAPPassword '<PASSWORD>' -CollectionMethod  <all | DcOnly | <COLLECTION_METHOD>

SharpHound.exe -v --Domain '<DOMAIN>' --domaincontroller '<DC_IP | DC_HOSTNAME>' --ldapusername '<USERNAME>' --ldappassword '<PASSWORD>' -c <all | DcOnly | <COLLECTION_METHOD>
```
